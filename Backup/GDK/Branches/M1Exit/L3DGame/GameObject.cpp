...
HRESULT GameObject::Load(_In_ Lucid3D::Screen* pScreen, _In_ uint64 id, _Out_ GameObjectPtr& spGameObject)
{
    HRESULT hr = S_OK;
    ...
    CHECKHR(pScreen->GetContentManager()->GetStream(id, &spStream));
    CHECKHR(Configuration::Load(spStream, &spProperties));

    {
        CHECKHR(Create(spProperties->GetStringValue("Name", "untitled"), spGameObject));
        ...
        for (size_t i = 0; i < _countof(componentNames); ++i)
        {
            ...
            if (componentId > 0)
            {
                CHECKHR(pScreen->GetScene()->CreateComponent(spGameObject, componentId, &spComponent));
            }
        }
    }
