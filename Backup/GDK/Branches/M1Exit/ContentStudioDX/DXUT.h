...
typedef void (CALLBACK *LPDXUTCALLBACKDEBUGPRINT)( char* pValue, void* pContext );
void DXUTSetDebugPrintCallback(LPDXUTCALLBACKDEBUGPRINT pCallback, void* pContext);
void __cdecl dxutodprintf(const char *format, ...);

#ifndef DXUTDEBUG_PRINT
#ifdef _DEBUG
    #define DXUTDEBUG_PRINT dxutodprintf
#else
    #define DXUTDEBUG_PRINT
#endif
#endif
...
HRESULT WINAPI DXUTSetWindow( HWND hWndFocus, HWND hWndDeviceFullScreen, HWND hWndDeviceWindowed, bool bHandleMessages = true, bool ownWindow = true );
...
void    WINAPI DXUTShutdown( int nExitCode = 0, bool closeWindow = true);
...
DirectX::XMMATRIX DXUTD3DXMatrixToXMMatrix(D3DXMATRIX& matrix, bool forceRH = false);
