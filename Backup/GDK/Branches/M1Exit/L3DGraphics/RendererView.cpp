...
GDK_METHODIMP_(const void*) RendererView::GetWindowIdentity() const
{
    return reinterpret_cast<const void*>(_hwnd);
}

GDK_METHODIMP_(bool) RendererView::IsFullScreen() const
{
    return _settings.IsFullScreen;
}

GDK_METHODIMP RendererView::SetFullScreen(_In_ bool fullScreen)
{
    return E_NOTIMPL;
}

GDK_METHODIMP RendererView::GetSize(_Out_ size_t* pWidth, _Out_ size_t* pHeight) const
{
    if (!pWidth || !pHeight)
        return E_INVALIDARG;
...
GDK_METHODIMP RendererView::SetSize(_In_ size_t width, _In_ size_t height)
{
    return E_NOTIMPL;
}
