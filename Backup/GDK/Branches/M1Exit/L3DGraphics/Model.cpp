...
HRESULT Model::Create(_In_ Renderer* pRenderer, _In_ IContentManagerPtr& spContentManager, _In_ uint64 contentId, _Out_ ModelPtr& spModel)
{
...

    {
        ...
        Mesh mesh;
        mesh.LocalTransform.Matrix = L3DMath::Matrix::Identity();
...
GDK_METHODIMP Model::Process(_Inout_ RenderMap& renderMap, _In_ GDK::Transform& transform)
{
    for (size_t i = 0; i < _meshes.size(); i++)
    {
        L3DMath::Matrix world(L3DMath::Matrix::Multiply(_meshes[i].LocalTransform.Matrix, transform.Matrix));
