...
GDK_METHODIMP Renderer::SetContentManager(_In_ IContentManager* pContentManager)
{
    _spContentManager.reset(pContentManager);
    return Geometry::CreateFullscreenQuad(this, GetContentManager(), _spQuad);
}

GDK_METHODIMP Renderer::Initialize(_In_ const GraphicsSettings& settings)
{
    HRESULT hr = S_OK;
...
GDK_METHODIMP Renderer::CreateView(_In_ const RendererViewSettings& viewSettings, _Deref_out_ IRendererView** ppView)
{
    HRESULT hr = S_OK;
...
GDK_METHODIMP Renderer::CreateScene(_Deref_out_ IRendererScene** ppScene)
{
    HRESULT hr = S_OK;
...
GDK_METHODIMP Renderer::GetViews(_Out_cap_(maxViews) IRendererView** views, _In_ size_t maxViews, _Out_ size_t* pNumViews)
{
    if (!views || !pNumViews)
    {
        return E_INVALIDARG;
    ...
    *pNumViews = stde::min(maxViews, _views.size());
...
GDK_METHODIMP Renderer::GetScenes(_Out_cap_(maxScenes) IRendererScene** scenes, _In_ size_t maxScenes, _Out_ size_t* pNumScenes)
{
    ...
    *pNumScenes = stde::min(maxScenes, _scenes.size());
...
void Renderer::CombineLighting()
{
    ...
    tasks.push_back(std::make_shared<RenderTask>(L3DMath::Matrix::Identity(), _spQuad, spMaterial, spShader));
