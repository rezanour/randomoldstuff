...
namespace Lucid3D
{
    ...
    // IRuntimeResource
    template <typename DerivedT>
    GDK_METHODIMP_(uint64) ResourceBase<DerivedT>::GetContentId() const
    {
        return _contentId;
    }

    template <typename DerivedT>
    GDK_METHODIMP_(bool) ResourceBase<DerivedT>::IsLoaded() const
    {
        return _isLoaded;
    }

    template <typename DerivedT>
    GDK_METHODIMP ResourceBase<DerivedT>::Load()
    {
        if (_isLoaded)
            return S_OK;
    ...
    template <typename DerivedT>
    GDK_METHODIMP ResourceBase<DerivedT>::Unload()
    {
        if (!_isLoaded)
            return S_OK;
