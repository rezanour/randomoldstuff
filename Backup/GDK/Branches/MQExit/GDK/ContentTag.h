...
namespace GDK
{
    ...
    {
        // Get content type
        GDK_METHOD_(ContentType::Enum)    GetType() const = 0;

        // Get sub type, if applicable
        GDK_METHOD_(ObjectType::Enum)     GetObjectType() const = 0;
        GDK_METHOD_(ComponentType::Enum)  GetComponentType() const = 0;
        GDK_METHOD_(AssetType::Enum)      GetAssetType() const = 0;
        GDK_METHOD_(ResourceType::Enum)   GetResourceType() const = 0;

        // A unique, unsigned 64bit value which can be used to refer to the content
        GDK_METHOD_(unsigned long long) GetID() const = 0;
