...
LightComponent::LightComponent(_In_ Renderer* pRenderer, _In_ IGameObject* pGameObject, _In_ GeometryPtr& spGeometry, _In_ const Lucid3D::Vector3& color)
    : _pRenderer(pRenderer), _pOwner(pGameObject), _spQuad(spGeometry), _color(color)
{
}

// IObjectComponent
GDK_IMETHODIMP_(const GDK::IGameObject*) LightComponent::GetOwner() const
{
    return _pOwner;
}

GDK_IMETHODIMP_(GDK::ObjectComponentType) LightComponent::GetType() const
{
    return ObjectComponentType::VisualComponent;
}

GDK_IMETHODIMP LightComponent::Process(_Inout_ RenderMap& renderMap)
{
