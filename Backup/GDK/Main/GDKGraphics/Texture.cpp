...
GDK_IMETHODIMP Texture::OnLoad(_In_ Renderer* pRenderer, _In_ IContentManagerPtr spContentManager, _In_ uint64 contentId)
{
    HRESULT hr = S_OK;
    ...
    _spSRV = nullptr;
    CHECKHR(GDKGraphics::TextureData::Load(spStream, spDevice, &_spSRV));

EXIT
    return hr;
}

GDK_IMETHODIMP Texture::OnUnload()
{
    HRESULT hr = S_OK;
