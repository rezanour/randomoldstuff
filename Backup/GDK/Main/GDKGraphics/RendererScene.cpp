...
GDK_IMETHODIMP_(bool) RendererScene::IsVisible() const
{
    return _isVisible;
}

GDK_IMETHODIMP_(void) RendererScene::SetVisible(_In_ bool visible)
{
    _isVisible = visible;
}

GDK_IMETHODIMP RendererScene::CreateComponent(_In_ GDK::IGameObject* pGameObject, _In_ uint64 contentId, _Deref_out_ GDK::IObjectComponent** ppComponent)
{
    HRESULT hr = S_OK;
...
GDK_IMETHODIMP RendererScene::RemoveComponent(_In_ GDK::IObjectComponent* pComponent)
{
...
GDK_IMETHODIMP RendererScene::SetActiveCamera(_In_ IObjectComponent* pComponent)
{
    if (!pComponent)
        return E_INVALIDARG;
...
GDK_IMETHODIMP RendererScene::AssociateView(_In_ IRendererView* view)
{
...
GDK_IMETHODIMP RendererScene::DisassociateView(_In_ IRendererView* view)
{
    stde::ref_counted_ptr<RendererView> spView;
...
GDK_IMETHODIMP RendererScene::GetAssociatedViews(_Out_cap_(maxViews) IRendererView** views, _In_ size_t maxViews, _Out_ size_t* pNumViews)
{
    if (!views || !pNumViews)
    {
        return E_INVALIDARG;
