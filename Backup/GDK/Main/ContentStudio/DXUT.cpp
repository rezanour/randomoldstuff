...
protected:
    struct STATE
    {
        ...
        // General
        BOOL  m_IsHWNDOwner;                // DXUT owns window
...
public:
    ...
    GET_SET_ACCESSOR( BOOL, IsHWNDOwner );
...
BOOL WINAPI DXUTGetIsHWNDOwner()                           { return GetDXUTState().GetIsHWNDOwner(); }
...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTSetWindow( HWND hWndFocus, HWND hWndDeviceFullScreen, HWND hWndDeviceWindowed, bool bHandleMessages, bool ownWindow )
{
    HRESULT hr;

    GetDXUTState().SetIsHWNDOwner( ownWindow );
...
void DXUTUpdateDeviceSettingsWithOverrides( DXUTDeviceSettings* pDeviceSettings )
{
    ...
    {
        if( GetDXUTState().GetOverrideForceAPI() == 11 )
        {
    ...
    if( pd3dDevice != NULL )
    {
        ...
        if( pd3dDevice )
        {
            ...
            if( references > 0 )
            {
                ...
                DEBUG_PRINT("ID3D11Device has %d outstanding references on final release", references);
            }
        }
...
//--------------------------------------------------------------------------------------
// Returns a string describing the current frame rate in FPS.
//--------------------------------------------------------------------------------------
LPCWSTR WINAPI DXUTGetFrameStatsOnly()
{
    WCHAR* pstrFrameStats = GetDXUTState().GetFPSStats();
    return pstrFrameStats;
}
...
//--------------------------------------------------------------------------------------
void WINAPI DXUTShutdown( int nExitCode, bool closeWindow )
{
    HWND hWnd = DXUTGetHWND();
    if( hWnd != NULL && closeWindow)
        SendMessage( hWnd, WM_CLOSE, 0, 0 );
...
DirectX::XMMATRIX DXUTD3DXMatrixToXMMatrix(D3DXMATRIX& matrix, bool forceRH)
{
    DirectX::XMMATRIX xm;
    for (int y = 0; y < 4; y++)
    {
