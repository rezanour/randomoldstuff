...
HRESULT ResourceFactory::CreateTextureResourceFromEmbeddedBitmapResource(PCSTR name, int resourceId, ITextureResource** ppResource)
{
    HRESULT hr = S_OK;
    stde::com_ptr<IContentStudioResourceFactory> spFactory;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            HRSRC hResource = FindResource(nullptr, MAKEINTRESOURCE(resourceId), MAKEINTRESOURCE(RT_BITMAP));
            if (hResource)
            {
                HGLOBAL hResourceMemory = LoadResource(NULL, hResource);
                if (hResourceMemory)
