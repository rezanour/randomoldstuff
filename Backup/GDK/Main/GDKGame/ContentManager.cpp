...
HRESULT ContentManager::Create(_In_ const std::string& contentRoot, _Inout_ ContentManagerPtr& spContentManager)
{
    ...
    FATAL(Configuration::Load(std::wstring(stde::to_wstring(contentRoot) + L"\\content.index"), &spConfiguration));
...
// IContentManager
GDK_IMETHODIMP ContentManager::GetContentTag(_In_ uint64 id, _Out_ IContentTag** ppContentTag)
{
    if (!ppContentTag)
        return E_POINTER;
...
GDK_IMETHODIMP ContentManager::RegisterRuntimeResource(_In_ GDK::IRuntimeResource* pResource)
{
    if (!pResource)
        return E_INVALIDARG;
...
GDK_IMETHODIMP ContentManager::UnregisterRuntimeResource(_In_ GDK::IRuntimeResource* pResource)
{
    if (!pResource)
        return E_INVALIDARG;
...
GDK_IMETHODIMP ContentManager::GetStream(_In_ uint64 id, _Deref_out_ IStream** ppStream)
{
    HRESULT hr = S_OK;

    std::wstring contentPath(_contentRoot);
    FileMap::iterator it = FileTableOfContents.find(id);
    FATAL(it != FileTableOfContents.end() ? S_OK : E_FAIL);
    ...
    FATAL(FileStream::Create(contentPath.c_str(), true, ppStream));
