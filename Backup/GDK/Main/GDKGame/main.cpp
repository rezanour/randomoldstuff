...
int RunGame(_In_ const wchar_t* commandLine)
{
    HRESULT hr = S_OK;
    ...
    FATAL(LoadConfiguration(std::wstring(commandLine), spConfiguration));
...
HRESULT LoadConfiguration(_In_ const std::wstring& commandLine, _Inout_ ConfigurationPtr& spConfiguration)
{
    ...
    if (!commandLine.empty())
    {
        ...
        FATAL(StringCchCopy(settingsFile, _countof(settingsFile), cmdLine.c_str()));
