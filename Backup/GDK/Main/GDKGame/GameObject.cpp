...
// IGameObject
GDK_IMETHODIMP_(unsigned long) GameObject::GetID() const
{
    return _id;
}

GDK_IMETHODIMP_(const char* const) GameObject::GetName() const
{
    return _name.c_str();
}

GDK_IMETHODIMP_(void) GameObject::SetName(_In_ const char* name)
{
    _name = name;
}

GDK_IMETHODIMP_(size_t) GameObject::GetTransformSalt() const
{
    return _transformSalt;
}

GDK_IMETHODIMP_(void) GameObject::GetTransform(_Out_ GDK::Transform* pTransform) const
{
    if (pTransform)
    {
...
GDK_IMETHODIMP_(void) GameObject::SetTransform(_In_ const GDK::Transform& transform)
{
...
GDK_IMETHODIMP GameObject::GetPropertyBag(_Deref_out_ GDK::IPropertyBag** ppProperties) const
{
    HRESULT hr = S_OK;
