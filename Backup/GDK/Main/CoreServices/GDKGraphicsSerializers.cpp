#include "StdAfx.h"
#include "GDKGraphicsSerializers.h"
#include "StreamHelper.h"
#include "Debug.h"

using namespace GDKGraphics;
...
HRESULT GeometryData::Load2(_In_ stde::com_ptr<IStream> spStream, _Out_ size_t* pStride, _Out_ size_t* pVertexCount, _Out_ std::vector<byte>& vertexData, _Out_ std::vector<uint32>& indices)
{
    ...
    *pVertexCount = geoHeader.VertexCount;
    ...
    size_t stride = sizeof(DirectX::XMFLOAT3) + sizeof(DirectX::XMFLOAT3) + sizeof(DirectX::XMFLOAT2);
    *pStride = stride;
    vertexData.resize(vertices.size() * stride);

    byte* pPositions = vertexData.data();
    byte* pNormals = vertexData.data() + sizeof(DirectX::XMFLOAT3);
    byte* pTexcoords = vertexData.data() + (2 * sizeof(DirectX::XMFLOAT3));

    for (size_t i = 0; i < vertices.size(); i++)
    {
        *reinterpret_cast<DirectX::XMFLOAT3*>(pPositions) = vertices[i].Position;
        *reinterpret_cast<DirectX::XMFLOAT3*>(pNormals) = vertices[i].Normal;
        *reinterpret_cast<DirectX::XMFLOAT2*>(pTexcoords) = vertices[i].TextureCoord0;
                                  ...
                                  _Deref_out_ ID3D11ShaderResourceView** ppView)
{
    ...
    textureDesc.Height = static_cast<UINT>(header.metadata.height);
    textureDesc.Width = static_cast<UINT>(header.metadata.width);
...

    {
        ...
        subData.SysMemPitch = static_cast<UINT>(image.rowPitch);
        subData.SysMemSlicePitch = static_cast<UINT>(image.slicePitch);
                                    ...
                                    _Out_ std::vector<GDK::ShaderConstant>& constants)
{
    ...
    if (FAILED(hr))
    {
        ...
        UNREFERENCED_PARAMETER(szErrors); // because DebugError can get omitted on release builds
        CHECKHR(hr);
    }
