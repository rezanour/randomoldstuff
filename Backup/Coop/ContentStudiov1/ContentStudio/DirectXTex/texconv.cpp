...
{
    wprintf( L" (%ux%u", info.width, info.height);

    if ( TEX_DIMENSION_TEXTURE3D == info.dimension )
        wprintf( L"x%u", info.depth);

    if ( info.mipLevels > 1 )
        wprintf( L",%u", info.mipLevels);
...
int __cdecl wmain(_In_ int argc, _In_z_count_(argc) wchar_t* argv[])
{
    ...
    for(int iArg = 1; iArg < argc; iArg++)
    {
        ...
        if(('-' == pArg[0]) || ('/' == pArg[0]))
        {
            ...
            {
            case OPT_WIDTH:
                if (swscanf_s(pValue, L"%u", &width) != 1)
                {
            ...
            case OPT_HEIGHT:
                if (swscanf_s(pValue, L"%u", &height) != 1)
                {
            ...
            case OPT_MIPLEVELS:
                if (swscanf_s(pValue, L"%u", &mipLevels) != 1)
                {
    ...
    for(pConv = pConversion; pConv; pConv = pConv->pNext)
    {
        ...
        if ( info.width != twidth || info.height != theight )
        {
            ...
            hr = Resize( image->GetImages(), image->GetImageCount(), image->GetMetadata(), twidth, theight, dwFilter, *timage );
            if ( FAILED(hr) )
            {
        ...
        if ( info.format != tformat && !IsCompressed( tformat ) )
        {
            ...
            hr = Convert( image->GetImages(), image->GetImageCount(), image->GetMetadata(), tformat, dwFilter, 05.f, *timage );
            if ( FAILED(hr) )
            {
        ...
        if ( (!tMips || info.mipLevels != tMips)
             && ispow2(info.width) && ispow2(info.height) && ispow2(info.depth) )
        {
            ScratchImage *timage = new ScratchImage;
            if ( !timage )
            {
            ...
            {
                hr = GenerateMipMaps3D( image->GetImages(), image->GetImageCount(), image->GetMetadata(), dwFilter, tMips, *timage );
            }
            else
            {
                hr = GenerateMipMaps( image->GetImages(), image->GetImageCount(), image->GetMetadata(), dwFilter, tMips, *timage );
            }
            if ( FAILED(hr) )
            {
