...
{
    m_DialogResourceManager.OnD3D11CreateDevice( pd3dDevice, DXUTGetD3D11DeviceContext() );
    return S_OK;
}
...
{
    *pbNoFurtherProcessing = m_DialogResourceManager.MsgProc( this->GetSafeHwnd(), uMsg, wParam, lParam);
    return 0;
}
...
HRESULT CDirect3DView::CompileShaderFromFile(WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut )
{
    HRESULT hr = S_OK;
    ...
    DEBUG_PRINT("Compiling shader file '%s', entry point '%s', shader model '%s'", szFileName, szEntryPoint, szShaderModel);
    hr = D3DX11CompileFromFile( szFileName, NULL, NULL, szEntryPoint, szShaderModel, 
        dwShaderFlags, 0, NULL, ppBlobOut, &pErrorBlob, NULL );
    GOTO_EXIT_IF_FAILED_MESSAGE(hr, "D3DX11CompileFromFile");

Exit:

    // Output any error information if it is provided
    if(FAILED(hr) && (pErrorBlob != NULL))
    {
        DEBUG_PRINT((char*)pErrorBlob->GetBufferPointer());
    }
