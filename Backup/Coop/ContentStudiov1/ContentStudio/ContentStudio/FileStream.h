...
class FileStream : public IStream
{
    ...
    FileStream(HANDLE hFile, BOOL bAttached) :
        _refcount(1),
        _hFile(hFile),
        _bAttached(bAttached)
    { 
    }
...
public:
    HRESULT static OpenFile(LPCWSTR pName, IStream ** ppStream, bool fWrite)
    {
        ...
        hFile = ::CreateFileW(pName, fWrite ? GENERIC_WRITE : GENERIC_READ, fWrite ? FILE_SHARE_WRITE : FILE_SHARE_READ, NULL, fWrite ? CREATE_ALWAYS : OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    HRESULT static AttachFile(HANDLE hFile, IStream ** ppStream)
    {
        *ppStream = new FileStream(hFile, TRUE);
        return S_OK;
    }
