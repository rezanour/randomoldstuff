...
HRESULT GetComponents(_Inout_updates_to_(maxComponents, *numComponents) ComponentDescription* components, _In_ size_t maxComponents, _Out_ size_t* numComponents)
{
    ...
    *numComponents = GetNumComponents();
    ...
    components[0].factoryId = __uuidof(IGraphicsDeviceFactory);
    components[0].internalId = __uuidof(OpenGLGraphicsDeviceFactory);

    size_t displayNameLength = wcslen(OpenGLGraphicsDevice::DisplayName) + 1;
    ...
    wcscpy_s(components[0].displayName, OpenGLGraphicsDevice::DisplayName);
...
{
    if (component.factoryId == __uuidof(IGraphicsDeviceFactory))
    {
        if (component.internalId == __uuidof(OpenGLGraphicsDeviceFactory))
        {
            return GDK::Make<OpenGLGraphicsDeviceFactory>().CopyTo(riid, factory);
        }
    }
