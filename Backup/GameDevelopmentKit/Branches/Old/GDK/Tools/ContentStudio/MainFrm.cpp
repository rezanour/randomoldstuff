...
{
    ComPtr<IGameFactory> factory;
    HRESULT hr = S_OK;

    // Get first IGameFactory in components list
    for (size_t i = 0; i < theApp._components.size(); i++)
    {
        if (theApp._components[i].factoryId == __uuidof(GDK::IGameFactory))
        {
            hr = ComponentFinderCreateComponentFactory(theApp._components[i], __uuidof(GDK::IGameFactory), &factory);
            if (SUCCEEDED(hr))
            {
                DebugOut(L"Created IGameFactory Successfully\n");
            }
            else
            {
...
void MainFrame::TestRun(_In_ IGameFactory* gameFactory)
{
    ComPtr<IGameEdit> game;
    ...
    parameters.content = nullptr;
    parameters.graphicsDevice = GetArchivePreviewRenderingPane()->GetGraphicsDevice();
    parameters.host = reinterpret_cast<IGameHost*>(dynamic_cast<ContentStudioView*>(GetActiveView()));
    hr = gameFactory->CreateGameEdit(parameters, &game);
    if (SUCCEEDED(hr))
    {
        hr = game->CreateWorld(L"E1M1", &world);
        if (SUCCEEDED(hr))
        {
            hr = world->CreateObject(L"enemies\\soldier.object", &gameObject);
            if (SUCCEEDED(hr))
            {
                for (size_t i = 0; i < gameObject->GetPropertyCount(); ++i)
                {
                    ComPtr<IProperty> prop;
                    gameObject->GetPropertyByIndex(i, &prop);

                    ComPtr<IString> string;
                    prop->ToString(&string);
                    DebugOut(L"Property: %s = %s\n", prop->GetName(), string->GetBuffer());

                    ComPtr<IStringProperty> stringProp;
                    ComPtr<IIntegerProperty> intProp;

                    if (SUCCEEDED(prop.As(&stringProp)))
                    {
                        DebugOut(L"    [string] : %s\n", stringProp->GetStringValue());
                    }
                    if (SUCCEEDED(prop.As(&intProp)))
                    {
                        DebugOut(L"    [int] : %d\n", intProp->GetIntegerValue());
                    }
