...
namespace GDK
{
    // Abstract base type for all file system volumes
    class FileVolume : NonCopyable
    {
    public:
    ...
    protected:
        FileVolume(_In_ const std::wstring &root, _In_ const std::wstring &logicalRoot, _In_ int32_t priority) : _realRoot(root), _logicalRoot(logicalRoot), _priority(priority)
        {
        }
