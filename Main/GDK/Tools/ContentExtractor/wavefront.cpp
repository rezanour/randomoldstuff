...
HRESULT SaveAsWaveFrontOBJ(const wchar_t* path, const byte_t* positions, size_t stride, size_t numVertices, const uint32_t* indices, size_t numIndices )
{
    ...
    for (size_t v = 0; v < numVertices; v++)
    {
        // Write vertex
        ZeroMemory(temp, sizeof(temp));
        GDK::Float3 position = *reinterpret_cast<const GDK::Float3*>(positions[v]);
        sprintf(temp, objVertex, position.x, position.y, position.z);
        objFileData.append(temp);
    }
