...
void ConvertMD2ToGeometryContent(_In_ std::vector<uint8_t>& md2, _Inout_ std::shared_ptr<GDK::GeometryContent>& content)
{
    ...
    Matrix rotate = Matrix::CreateRotationY(XMConvertToRadians(-90.0f));
    Vector3 pos;
    ...
    for (int frameIndex = 0; frameIndex < fileHeader->num_frames; frameIndex++)
    {
        ...
        for (int t = 0; t < fileHeader->num_tris; t++)
        {
            ...
            for (int v = 0; v < 3; v++)
            {
                ...
                pos = Vector3::Transform(pos, rotate);
    ...
    GenerateNormals(reinterpret_cast<const uint8_t*>(&allVertices[0].position), sizeof(geometryContentVertex_t), allVertices.size(), allIndices.data(), allIndices.size(), reinterpret_cast<uint8_t*>(&allVertices[0].normal));
    ...
    vertexData.reset(new uint8_t[sizeof(geometryContentVertex_t) * allVertices.size()]);
