...
namespace GDK
{
    ...
    std::shared_ptr<TextureContent> TextureContent::Create(_In_ uint32_t width, _In_ uint32_t height, _In_ TextureFormat format, _Inout_ std::unique_ptr<uint8_t[]>& pixels, _In_ uint32_t numFrames, _Inout_ std::unique_ptr<RectangleF[]>& frames)
    {
        return std::shared_ptr<TextureContent>(new TextureContent(width, height, format, pixels, numFrames, frames));
    }
    ...
    std::shared_ptr<TextureContent> TextureContent::Create(_Inout_ std::istream &stream)
    {
        ...
        std::unique_ptr<uint8_t[]> pixels(new uint8_t[header.ImageSize]);
    ...
    TextureContent::TextureContent(_In_ uint32_t width, _In_ uint32_t height, _In_ TextureFormat format, _Inout_ std::unique_ptr<uint8_t[]>& pixels, _In_ uint32_t numFrames, _Inout_ std::unique_ptr<RectangleF[]>& frames) :
        _width(width),
        _height(height),
        _format(format),
    ...
    const uint8_t* TextureContent::GetPixels() const
    {
        return _pixels.get();
    }
