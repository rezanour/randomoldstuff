    ...
    if (inputfile.is_open())
    {
        ...
        for(size_t i = 0; i < entries.size(); i++)
        {
            ...
            else
            {
                ...
                {
                    printf("NOT - converting '%ws' to '%ws'\n", entryName.c_str(), outputName.c_str());
                }
...
void ExtractAllWolf3DContent(_In_ const std::wstring& path, _In_ const std::wstring& outputDirectory)
{
    ...
    if (g_extractWL6_WALL)
    {
        ...
        for (size_t i = 0; i < vswapChunks.size(); i++)
        {
            ...
            {
                bmpName.append(L".bmp");
                bmp_infoheader_t* bmfh = (bmp_infoheader_t*)(inputbuffer.data() + sizeof(bmp_fileheader_t));
                bmfh->height = -bmfh->height;
                WriteBufferToFile(bmpName, inputbuffer);
            }
        }
    ...
    if (g_extractWL6_MAP)
    {
        ...
        for (size_t i = 0; i < /*levels.size()*/1; i++)
        {
            ...
            ReadW3dLevelLayerToBuffer(gamemapsBuffer, levels[i], itemsLayer ,levelItems);

            ConvertWallsLayerToGeometryContentList(levelItems, level, textureNameList, contentList, itemsList, g_scaleFactor);

            for (size_t c = 0; c < contentList.size(); c++)
            {
                ...
                ReplaceCharacter(textureNameList[c], L'\\', L'%');
                geometryName.append(textureNameList[c]);
                geometryName.append(L".geometry");

                WriteGeometryToFile(geometryName, contentList[c]);

                //std::wstring geometryObjName = geometryName;
                //geometryObjName += L".obj";
                //WriteGeometryToObjFile(geometryObjName, contentList[c]);
            }

            // Read map items
            ConvertItemsLayerToItemsList(levelItems, itemsList, g_scaleFactor);
            std::vector<uint8_t> itemsBuffer;
            for (auto str : itemsList)
            {
                std::string tmp = str;
                tmp += "\r\n";
                for(size_t ch = 0; ch < tmp.size(); ch++)
                {
                    itemsBuffer.push_back((uint8_t)tmp[ch]);
                }
            }

            std::wstring itemsName = outputName;
            itemsName.append(L"\\items.txt");
            WriteBufferToFile(itemsName, itemsBuffer);
        }
    }
    ...
    if (g_generateWL6_Models)
    {
        ...
        std::shared_ptr<GDK::GeometryContent> pushWallContent;
        CreatePushWallModel(g_scaleFactor, pushWallContent);

        outputName = outputDirectory + L"\\models\\Doors\\Pushwall";
        CreateFullDirectory(outputName);
        outputName.append(L"\\w3d_pushwall.geometry");

        WriteGeometryToFile(outputName, pushWallContent);
    }
}
