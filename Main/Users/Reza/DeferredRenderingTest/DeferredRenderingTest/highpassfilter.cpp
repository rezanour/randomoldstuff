...
HighPassFilter::HighPassFilter(ID3D11DeviceContext* context, const std::shared_ptr<Texture2D>& source, const std::shared_ptr<Texture2D>& dest) :
    _context(context), _source(source), _dest(dest)
{
    ...
    _blurScratch.reset(new Texture2D(device.Get(), td.Width, td.Height, D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE, td.Format));

    _vertexShader = LoadVertexShader(device.Get(), L"vsCombine.cso");
    _pixelShader = LoadPixelShader(device.Get(), L"psHighPass.cso");
...
void HighPassFilter::RenderFrame(const std::shared_ptr<Camera>& camera, const std::shared_ptr<Scene>& scene)
{
    ...
    _blur->BlurSurface(_dest, _blurScratch);
}
