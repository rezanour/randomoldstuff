...
void DistortionMeshDestroy ( DistortionMeshVertexData *pVertices, UInt32 *pTriangleMeshIndices )
{
    OVR_FREE ( pVertices );
    OVR_FREE ( pTriangleMeshIndices );
}

void DistortionMeshCreate ( DistortionMeshVertexData **ppVertices, UInt32 **ppTriangleListIndices,
                           ...
                           const DistortionRenderDesc &distortion, const ScaleAndOffset2D &eyeToSourceNDC )
{
    ...
    *ppTriangleListIndices  = (UInt32*) OVR_ALLOC( sizeof(UInt32) * (*pNumTriangles) * 3 );
    ...
    UInt32 *pcurIndex = *ppTriangleListIndices;
    ...
    for ( int triNum = 0; triNum < DMA_GridSize * DMA_GridSize; triNum++ )
    {
        ...
        {
            *pcurIndex++ = (UInt32)FirstVertex;
            *pcurIndex++ = (UInt32)FirstVertex+1;
            *pcurIndex++ = (UInt32)FirstVertex+(DMA_GridSize+1)+1;

            *pcurIndex++ = (UInt32)FirstVertex+(DMA_GridSize+1)+1;
            *pcurIndex++ = (UInt32)FirstVertex+(DMA_GridSize+1);
            *pcurIndex++ = (UInt32)FirstVertex;
        }
        else
        {
...
void HeightmapMeshDestroy ( HeightmapMeshVertexData *pVertices, UInt32 *pTriangleMeshIndices )
{
    OVR_FREE ( pVertices );
    OVR_FREE ( pTriangleMeshIndices );
}

void HeightmapMeshCreate ( HeightmapMeshVertexData **ppVertices, UInt32 **ppTriangleListIndices,
    int *pNumVertices, int *pNumTriangles,
    const StereoEyeParams &stereoParams, const HmdRenderInfo &hmdRenderInfo )
{
    bool    rightEye      = ( stereoParams.Eye == StereoEye_Right );
    ...
    *ppTriangleListIndices  = (UInt32*) OVR_ALLOC( sizeof(UInt32) * (*pNumTriangles) * 3 );
    ...
    for ( int triNum = 0; triNum < HMA_GridSize * HMA_GridSize; triNum++ )
    {
        ...
        if ( ( x < HMA_GridSize/2 ) != ( y < HMA_GridSize/2 ) )       // != is logical XOR
        {
            *pcurIndex++ = (UInt32)FirstVertex;
            *pcurIndex++ = (UInt32)FirstVertex+1;
            *pcurIndex++ = (UInt32)FirstVertex+(HMA_GridSize+1)+1;

            *pcurIndex++ = (UInt32)FirstVertex+(HMA_GridSize+1)+1;
            *pcurIndex++ = (UInt32)FirstVertex+(HMA_GridSize+1);
            *pcurIndex++ = (UInt32)FirstVertex;
        }
        else
        {
