...
//        LogText("GenerateDistortion time = %f\n", deltaT);
		...
		DistortionMeshIBs[eyeNum]->Data(Buffer_Index | Buffer_ReadOnly, meshData.pIndexData, (sizeof(UInt32)* meshData.IndexCount));
    ...
    {        
		...
		{                       
            ...
            renderPrimitives(&distortionShaderFill, DistortionMeshVBs[eyeNum], DistortionMeshIBs[eyeNum],
                            NULL, 0, (int)DistortionMeshIBs[eyeNum]->GetSize() / 4, Prim_Triangles);
		}
        else
        {
            renderPrimitives(&distortionShaderFill, DistortionMeshVBs[eyeNum], DistortionMeshIBs[eyeNum],
                          ...
                          PrimitiveType rprim)
{
    ...
    if (indices)
    {
        RParams.pContext->IASetIndexBuffer(indices->GetBuffer(), DXGI_FORMAT_R32_UINT, 0);
    }
